...
public class ProductController {
    ...
    @RequestMapping(value = "/items/{storeId}", method = RequestMethod.GET)
    public ResponseEntity<List<Item>> getAllItems(@PathVariable String storeId) throws ShoppersException {
        List<Item> items = new ArrayList<>();
        try {
            items = itemService.getItemsByStoreId(storeId);
        }catch (Exception e){
            throw new ShoppersException("Error occurred while updating inventory.");
        }
        return new ResponseEntity<>(items, HttpStatus.OK);
